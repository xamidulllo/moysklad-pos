...
async def _test_daily_order_status(business_day: str, _: dict = Depends(get_current_session)):
    """VAQTINCHALIK diagnostika (2026-08-30) — faqat o'qiydi, hech narsa
    o'zgartirmaydi. Berilgan ish kuni uchun "DailyOrders" qatorining
    hozirgi holatini (status, xato matni, zakaz/otgruzka ID'lari) qaytaradi
    — "needs_manual_check" holatida qayta sinxronlash uchun avval buni
    ko'rish kerak (haqiqatan zakaz yaratilganmi yoki yo'qmi bilish uchun).
...
async def _test_reset_daily_order(business_day: str, _: dict = Depends(get_current_session)):
    ...
    tegishli (noto'g'ri valyuta bilan yaratilgan) eski zakazni QO'LDA
    o'chirib tashlaganini tasdiqlagandan keyin chaqirilishi kerak — aks
    holda keyingi sinxronizatsiya DUBLIKAT zakaz yaratib qo'yishi mumkin.
    Ishlatilgach OLIB TASHLANADI."""
    daily = await sheets_client.get_daily_order(business_day)
    if daily is None:
